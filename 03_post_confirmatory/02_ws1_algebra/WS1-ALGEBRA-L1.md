...
Pós-firewall (§7): a análise prévia de agente único chegou, por via distinta, a
uma condição PROVADAMENTE equivalente (τ ∈ Iso(W); s14 frente 1: det −8, posto
6); a tentativa activa de refutação falhou em três frentes; ficam duas
derivações e duas OOS independentes (910000004 prévia, 910000005 WS1), 0
excepções em 40000 verificações.
...
A firewall foi levantada só depois de NOTAS-DERIVACAO.md, s10-s13, todos os
outputs e o rascunho deste relatório (secções 1-6, 8-9) estarem escritos e no
servidor (`WS1-ALGEBRA-L1-DRAFT-prefirewall.md`, sha256 `3b57fbab…a1d1ef`,
13:55 UTC). Só então foram lidos `condicao_L1.py`, `condicao_L1_oos.py`,
`condicao-L1-insample.json`, `condicao-L1-oos.json`,
`precommit-oos-condicaoL1.txt` e `FASE6-AUTOPSIA-7bb0baab3a8ed7aa.md`.

### 7.1 O que a análise prévia afirma

Condição prévia (por aresta): **K_prev ⟺ W̃_0 == W̃_1 ⟺ τ = π1∘π0⁻¹ ∈
Iso(W_M)** — τ é isometria da "geometria de resposta" W_M(p,q) =
Σ_r pc(M[r][p]⊕M[r][q]) (M=G0 ou F0), com o teorema K_prev ⟺ L1 (necessidade
por um sistema linear; suficiência pela fórmula d_m(a) = 32·Σ_c W̃_m(c,
sub_a(c))). Validação prévia: 0 excepções em 20000 arestas in-sample
(TP 370 C_AB→B + 335 C_BA→A) e OOS pré-comprometida com semente 910000004,
N=5000 (TP 187+181, FP=FN=0); fam20 com φ bem definida, W̃ iguais nas duas
arestas; tabela por classe de τ; modelo de correlação por τ partilhado
(lift 1,91-1,92 vs 2,34-2,36 observado).

### 7.2 Reconciliação formal: as duas condições são A MESMA (demonstrado)

- Convenções: a análise prévia usa τ = π1∘π0⁻¹ (lado da IMAGEM de π); WS1 usa
  ρ = π0⁻¹∘π1 (lado do DOMÍNIO do canal). Vale τ = π0∘ρ∘π0⁻¹ — conjugadas,
  mesma classe de ciclo, e τ ∈ Iso(W_M) ⟺ ρ ∈ Iso(W_0) ⟺ Δ := W_0−W_1 = 0.
- K_prev exige Δ=0 nas 6 entradas; K_WS1 exige só os agregados
  (a) Δ01+Δ23=0, (b) Δ02+Δ13=0, (c) somas-linha de Δ nulas. **s14 (frente 1)
  prova por eliminação inteira (Bareiss) que esse sistema 6×6 tem determinante
  −8 ≠ 0 e posto 6: núcleo {0}. Logo K_WS1 ⟺ Δ=0 ⟺ K_prev.** As duas equipas
  chegaram à MESMA condição por formas equivalentes — e, de facto, o passo de
  necessidade da análise prévia usa exactamente os funcionais agregados de WS1
  (somas-linha + os dois pares), pelo que ambas as derivações factorizam pelo
  mesmo sistema linear; diferem na forma-título (isometria vs agregados).
- Verificação cruzada de números (independente, antes da leitura): os meus
  totais L1 por aresta somam 705 = 370+335 deles; a tabela por classe é
  idêntica (n=2615/1321/3451/2613; taxas iguais); o modelo de correlação dá
  1,91 em ambos; na fam20, as matrizes W̃ publicadas por eles implicam
  exactamente os meus agregados (A=B=WM3=8 nas duas arestas; V=(14,10,12,12)
  e (12,12,12,12)) e o "τ = única isometria não trivial (troca de 2 símbolos)"
  deles é o meu ρ' único de classe transposição.

### 7.3 Tentativa activa de refutação (s14) — falhou nas três frentes

1. **Formal**: se o sistema (a,b,c) tivesse posto <6, K_WS1 seria mais fraca
   que K_prev e existiriam contra-exemplos entre elas. Posto = 6 (det −8):
   equivalência exacta; nenhuma das duas está "com folga".
2. **Casos-canto construídos** (414 instâncias determinísticas: F0 constantes,
   sem dependência do canal, colunas iguais, latinas, esparsas, perfis de
   empate × 23 ρ × 2 σA): 0 contra-exemplos a K_prev==K_WS1, a K⟺L1 e à
   fórmula compacta prévia; distribuição obtida 178 L1 / 0 L2 / 236 L3 com 92
   casos de W≡0 (degenerescência total — K e L1 triviais, coerente).
3. **Busca em massa**: 20000 arestas in-sample (recomputadas por mim,
   independentemente) + 10000 OOS minhas (semente 910000005) + 10000 OOS
   deles (semente 910000004, lida dos outputs publicados): **0 excepções em
   40000 verificações aresta-a-aresta com 4 sementes**. Nenhum contra-exemplo
   encontrado em parte alguma.

Única imprecisão detectada (cosmética, sem efeito nos dados): a frase de
§4.5 da autópsia "K falso (τ fora do grupo) → L3" — em rigor K falso ⟹ ¬L1 =
L2 ∪ L3 (as 225 arestas L2 in-sample têm K falso). As matrizes de confusão
prévias tratam o caso correctamente (L2 conta como TN); é só a prosa que
abrevia.

### 7.4 Contribuições distintas (complementares, não conflituantes)

- Só na análise prévia: separação da confusão por aresta; estatística
  "|Iso(W)|>1 mas K falso" (~46% — simetria de W sozinha não chega); controlos
  emparelhados com o MESMO τ (um L1, um L3); nota de que o resíduo do lift
  pode incluir tilt de elegibilidade (E2/E6 acoplam F0,G0,π).
- Só em WS1: caracterização ordinal completa (Prop. 8) com validação OOS
  perfeita também para L1∪L2 e para o colapso por instância; **Corolário 8.1
  (conservação de histograma: ρ∈V4 ⟹ L2 impossível — 0 L2 em 3958 arestas V4
  das minhas amostras)**; fórmula e caracterização do dep (dep=0 ⟺
  invariância de padrões; caso fam 4159); elo directo com `cl.classificar`
  (s12b: 252/252 C1p, 126/126 fusão); reprodução cega dos VECTORES d completos
  e do dep=4608 da fam20 (a prévia verificou φ e W̃; WS1 verificou também os
  d e o dep registados).
- Consistências de enquadramento: o meu I2 (P(≥1 falha em 75 II) ≈ 27-29%)
  coincide com o desdobramento prévio (P(≥1 em 25 II E2) ≈ 10,9%;
  P(0 em 50 E1) ≈ 79,4%).

### 7.5 Veredicto da comparação

Concordância integral. A condição prévia e a de WS1 são formulações
equivalentes (demonstração formal em s14) da mesma condição necessária e
suficiente, agora validada por DUAS derivações independentes e DUAS validações
fora-da-amostra pré-comprometidas em sementes distintas, sem uma única
excepção. A conclusão prévia sobrevive à tentativa de refutação; WS1
acrescenta-lhe a camada ordinal (L2/fusão), o corolário V4-sem-L2, a
caracterização do dep e o fecho com o classificador real.
...
Q5. (Herdada da análise prévia, continua aberta) Comportamento de um teste
    estrito restringido a configurações realizadas em AMBOS os contextos de
    memória, e a especificidade em III sob operacionalizações mais finas —
    fora do âmbito de WS1.

## 9. Reprodutibilidade e inventário

- Derivação independente: `NOTAS-DERIVACAO.md` (pré-firewall); rascunho
  arquivado pré-firewall: `WS1-ALGEBRA-L1-DRAFT-prefirewall.md`.
- Scripts: `s10_exemplos_validacao.py`, `s11_fam20_cega.py`,
  `s12_replay_completo.py`, `s12b_classificar_real.py`, `s13_oos.py`,
  `s14_refutacao.py` (pós-firewall).
- Outputs: `out-s10.json`, `out-s11.json`, `out-s12.json`, `out-s12b.json`,
  `out-s13.json`, `out-s14.json`, logs `s10.log`…`s14.log`.
- Pré-registo OOS: `precommit-ws1-oos.txt` (sha256 `454821ad…8166`, escrito e
  enviado ANTES de s13; semente 910000005). Sementes 910000006-9: RESERVADAS e
  NÃO usadas (a refutação s14 foi determinística, sem amostras novas).
- Sementes usadas por WS1: apenas 910000005 (nova); 910000001/910000002 são
  reanálise determinística de datasets existentes; 910000004 (OOS prévia) NÃO
  foi recorrida — os seus resultados foram lidos dos outputs publicados.
  Nenhuma semente proibida foi tocada; nada em `/root/causal-A-amd2-official/`
  foi lido ou escrito; instrumento congelado intacto (12/12 vs MANIFEST).
