...
> candidato, caracterizar o seu conjunto de invisibilidade 𝓑_F(X), e quando
> aplicável a respetiva estrutura de simetria/estabilizador, e estimar a
> medida de configurações não-identificáveis sob a lei do gerador.

Simbolicamente:

    𝓑_F(X) = { g ∈ G : F(g·X) = F(X) }

> 𝓑_F(X) é o **conjunto de invisibilidade** (*blind set*) do observável F
> relativamente à configuração/estrutura X.

- não se assume que 𝓑_F(X) seja um grupo — para um X particular, este
  conjunto não tem necessariamente estrutura de grupo;
- quando possui estrutura de estabilizador/subgrupo, pode adicionalmente
  falar-se num **grupo de invisibilidade**;
- no caso concreto estudado, a estrutura relevante pode depender de W_M,
  portanto do mecanismo/receptor concreto.

— **princípio metodológico futuro, não novo critério C1″.** Esta correção de
notação não constitui uma nova contribuição científica.
...
| P4 | Na construção estudada, a falsa invariância altera a partição ESTADO/SINAL produzida por C1′ apesar de existir informação causal separadora realizada | caso demonstrado + interpretação conceptual; novidade pendente | **máxima** |
| P5 | teorema do alinhamento/L2 | novidade pendente | alta |
| P6 | III-1: nulidade estrutural em h=1 | novidade pendente | alta |
| P7 | A nulidade estrutural exata de III em h=1 para o campo XOR-intervencional não é preservada sob T∘T; em h=2, dep2>0 em 3996/4000 arestas III sob a lei estudada | resultado analítico + empírico; novidade pendente | **máxima** |
| P8 | individuação observada = f(sistema, observável, horizonte) | interpretação; não teorema | conceptual |

**Nota obrigatória sobre P4 — distinção facto/interpretação.**
*Facto demonstrado:* na família 20, o sistema é II; existem módulos
causalmente distintos; existe dependência causal real e estritamente
realizada; C1′ funde o núcleo e produz a partição correspondente a III.
*Interpretação conceptual:* isto sustenta a leitura de que, para C1′, a
individuação produzida depende do par {sistema, observável} — formulação que
permanece **INTERPRETAÇÃO CONCEPTUAL — NÃO TEOREMA UNIVERSAL**. Não se
escreve "a individuação causal em geral depende do observável".

**Nota obrigatória sobre P7 — separação analítica/empírica.**
*Parte analítica:* em h=1 o termo de memória relevante cancela exatamente no
contraste XOR (nulidade estrutural de III para o campo considerado); em h=2 a
composição faz a dependência de memória reaparecer dentro dos índices
internos — o mecanismo de cancelamento de primeira ordem **não itera
genericamente**. *Parte empírica:* na amostra estudada, 3996/4000 = 99,90%
das arestas III apresentam dep2>0; **existem 4/4000 exceções**. Não se
escreve: "III é sempre dependente em h=2"; "provámos que todo III deixa de
ser específico em h=2"; "nenhuma extensão por horizonte pode funcionar".
Designação principal: **não-estabilidade temporal da invariância de III**;
consequência: **obstrução a extensões diretas por simples aumento de
horizonte**.
...
> resposta preservada pelo observável. Na construção estudada, essa
> invisibilidade alterou a partição ESTADO/SINAL produzida por C1′.
> Separadamente, a especificidade
