...
def cronometrar(seleccao, dir_e1, dir_e2, amostra=None):
    ...
    for grupo in ordem:
        ids = seleccao[grupo]
        # Pre-data Amendment No. 2: carregar exactamente os três IDs
        # seleccionados; iniciar o relógio; executar os três EM SIMULTÂNEO
        # através da camada paralela de equivalencias.py, com exactamente
        # WORKERS_EQUIV=3 workers; aguardar os três; descartar os resultados
        # classificatórios (só o wall-clock total do batch sobrevive). Cada
        # testar_instancia() continua exaustiva e sequencial dentro do worker.
        instancias = [json.load(open(indice[iid])) for iid in ids]
        t0 = time.perf_counter()
        # API de produção: SEMPRE W=3 + worker científico real (sem escolher
        # workers nem injectar worker). Resultados descartados; só o relógio.
        eq.testar_instancias_em_lote(instancias, amostra=amostra)
